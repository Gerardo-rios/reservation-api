...
def dependencies_factory(
    ...
    def _dependencies_factory(role: Optional[entities.Role] = None) -> Dict[str, Any]:
        ...
        role_repository_mock.get_by_name.return_value = role
        role_repository_mock.get_by_id.return_value = role
        return {
...
def test__get_role_use_case__returns_a_role__when_successfully(
    ...
    dependencies["role_repository"].get_by_name.assert_called_once()
...
def test__get_role_use_case__returns_a_role_by_id__when_successfully(
    mocker: MockFixture,
    fixture_role_data: Dict[str, Any],
    dependencies_factory: Callable[[Optional[entities.Role]], Dict[str, Any]],
) -> None:
    ...
    dependencies = dependencies_factory(role)
    use_case = use_cases.GetRoleUseCase(**dependencies)
    request_input = request_models.GetRoleRequest(role_id=role.role_id)
    request_response = response_models.GetRoleResponse(
        role_id=role.role_id, role_name=role.role_name, description=role.description
    )
    response = use_case.execute(request_input)

    dependencies["role_repository"].get_by_id.assert_called_once_with(role.role_id)
    assert response == request_response


def test__get_role_use_case__raises_exception__when_role_id_not_found(
    mocker: MockFixture,
    dependencies_factory: Callable[[Optional[entities.Role]], Dict[str, Any]],
) -> None:
    dependencies = dependencies_factory(None)
    use_case = use_cases.GetRoleUseCase(**dependencies)
    input_dto = request_models.GetRoleRequest(role_id="invalid_role_id")

    with pytest.raises(errors.ItemNotFoundException) as exc_info:
        use_case.execute(input_dto)

    assert str(exc_info.value) == "Role 'invalid_role_id' was not found"
    dependencies["role_repository"].get_by_id.assert_called_once_with("invalid_role_id")
